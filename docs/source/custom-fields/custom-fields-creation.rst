...
Crm.Kontur позволяет добавлять пользовательские поля в основные сущности системы: 
...
Создание пользовательских полей через интерфейс
-----------------------------------------------


Для того, чтобы добавить в сделку поле нужно:

1. С правами Администратора перейти в карточку сделки
   
2. Перейти в настройки и выбрать «Добавить поле»

.. image:: /_static/custom-fields-menu.png

3. Заполняем настройки. Нажимаем «Сохранить». 

.. image:: /_static/new-field.png

4. После добавления поля, в выбираем в Настройках (п.2) настроить «Настроить порядок полей»

.. image:: /_static/fields-order.png

5. Перетаскиваем поле выше «Показать еще», чтобы всегда видеть его при открытии карточки сделки.

6. Переходим на «Список сделок» и если нам необходимо добавляем поле для «вывода» на списке.

.. image:: /_static/columns-settings.png
